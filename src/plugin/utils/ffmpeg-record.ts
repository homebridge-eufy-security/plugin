...
import { FfmpegProcess } from './ffmpeg-new';
...
export class FfmpegRecordingProcess extends FfmpegProcess {
  ...
  constructor(camera: CameraAccessory, recordingConfig: CameraRecordingConfiguration, rtspEntry: RtspEntry, isAudioActive: boolean) {
    ...
    if(camera.timeshift) {
      ...
      this.commandLineArgs.push(
        ...
        ...this.camera.stream!.ffmpegOptions.videoDecoder,
        '-probesize', this.camera.stream!.probesize.toString(),
        '-max_delay', '500000',
        '-r', rtspEntry.channel.fps.toString(),
        '-rtsp_transport', 'tcp',
        '-i', rtspEntry.url,
    ...
    this.commandLineArgs.push(
      ...
      '-map', '0:v:0',
      ...this.camera.stream!.ffmpegOptions!.recordEncoder(
        recordingConfig.videoCodec.resolution[0]!,
        recordingConfig.videoCodec.resolution[1]!,
        recordingConfig.videoCodec.resolution[2]!,
        recordingConfig.videoCodec.parameters.bitRate!,
        recordingConfig.videoCodec.parameters.profile!,
        recordingConfig.videoCodec.parameters.level!,
        recordingConfig.videoCodec.parameters.iFrameInterval / 1000!,
        rtspEntry.channel.fps,
      ),
      '-reset_timestamps', '1',
    ...
    if(this.camera.platform.verboseFfmpeg) {
  ...
  // Log errors.
  protected override logFfmpegError(exitCode: number, signal: NodeJS.Signals): void {
