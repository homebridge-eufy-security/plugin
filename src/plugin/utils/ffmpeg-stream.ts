...
import { FfmpegProcess } from './ffmpeg-new.js';
...
export class FfmpegStreamingProcess extends FfmpegProcess {
  ...
  constructor(delegate: StreamingDelegate, sessionId: string, commandLineArgs: string[], returnPort?: { addressVersion: string; port: number },
  ...
  private createSocket(portInfo: { addressVersion: string; port: number }): void {
    ...
    socket.on('message', messageListener = (): void => {
      ...
      this.streamTimeout = setTimeout(() => {
        ...
        this.log.debug('Video stream appears to be inactive for 5 seconds. Stopping stream.', this.camera.accessory.displayName);
