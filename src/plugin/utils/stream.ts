...
import { FfmpegOptions } from './ffmpeg-options.js';
import { FfmpegStreamingProcess } from './ffmpeg-stream.js';
import { Logger as TsLogger, ILogObj } from 'tslog';
import { EufySecurityPlatform } from '../platform';
import { RecordingDelegate } from './record';
import { RtpDemuxer } from './rtp.js';
...
export class StreamingDelegate implements CameraStreamingDelegate {
  ...
  public hksv: RecordingDelegate | null;
  ...
  constructor(camera: CameraAccessory, resolutions: [number, number, number][]) {
    ...
    this.ffmpegOptions = new FfmpegOptions(camera);

    // Setup for HKSV, if enabled.
    if (this.camera.cameraConfig.hsv) {
      this.log.debug(`${this.camera.name} has HKSV enabled`);
      this.hksv = new RecordingDelegate(camera);
    }
  ...
  public async prepareStream(request: PrepareStreamRequest, callback: PrepareStreamCallback): Promise<void> {
    ...
    const response: PrepareStreamResponse = {
      ...
      audio: {
        ...
        port: (hasAudioSupport && this.camera.cameraConfig.talkback) ? audioIncomingPort : audioIncomingRtcpPort,
  ...
  private async startStream(request: StartStreamRequest, callback: StreamRequestCallback): Promise<void> {
    ...
    const isTranscoding = false;

    // Find the best RTSP stream based on what we're looking for.
    this.rtspEntry = this.camera.findRtsp(
      (isTranscoding && this.camera.hardwareTranscoding) ? 3840 : request.video.width,
      (isTranscoding && this.camera.hardwareTranscoding) ? 2160 : request.video.height,
      undefined,
    ...
    if (!this.savedBitrate) {
      ...
      this.savedBitrate = this.camera.getBitrate(this.rtspEntry.channel.id);
    ...
    await this.camera.setBitrate(this.rtspEntry.channel.id, request.video.max_bit_rate * 1000);
    ...
    this.log.info('Streaming request from %s%s: %sx%s@%sfps, %s kbps. %s %s, %s kbps.',
      ...
      request.video.width, request.video.height, request.video.fps, request.video.max_bit_rate.toLocaleString('en-US'),
      isTranscoding ? (this.camera.hardwareTranscoding ? 'Hardware accelerated transcoding' : 'Transcoding') : 'Using',
    ...
    const ffmpegStream = new FfmpegStreamingProcess(this, request.sessionID, ffmpegArgs,
      (sessionInfo.hasAudioSupport && this.camera.cameraConfig.talkback) ? undefined :
    ...
    const sdpReturnAudio = [
      ...
      's=' + this.camera.accessory.displayName + ' Audio Talkback',
    ...
    const ffmpegReturnAudioCmd = [
      ...
      '-flags', '+global_header',
      '-ar', request.audio.sample_rate.toString(),
  ...
  public async getSnapshot(request?: SnapshotRequest, isLoggingErrors = true): Promise<Buffer | null> {
    ...
    const snapshot = fs.readFileSync(CameraOffline);
    ...
    if (!snapshot) {
      ...
      const cachedSnapshot = this.getCachedSnapshot(this.camera.accessory.UUID);
    ...
    this.snapshotCache[this.camera.accessory.UUID] = { image: snapshot, time: Date.now() };
    return this.snapshotCache[this.camera.accessory.UUID].image;
  }
      ...
      if (!this.ongoingSessions['length']) {
        ...
        if (this.hksv?.isRecording) {
          ...
          if (this.rtspEntry) {
            ...
            await this.camera.setBitrate(this.rtspEntry.channel.id, this.savedBitrate);
          }
  ...
  public get probesize(): number {
    ...
    // return this.probesizeOverride ? this.probesizeOverride : this.camera.cameraConfig.videoConfig.probesize;
    return this.probesizeOverride;
  }
}