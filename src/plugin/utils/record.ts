...
import { FfmpegRecordingProcess } from './ffmpeg-record';
import { PROTECT_HKSV_MAX_EVENT_ERRORS } from '../settings.js';
import { Logger as TsLogger, ILogObj } from 'tslog';
import { TimeshiftBuffer } from './timeshift';
...
export class RecordingDelegate implements CameraRecordingDelegate {
  ...
  private readonly timeshift: TimeshiftBuffer;
  ...
  constructor(camera: CameraAccessory) {
    ...
    this.camera = camera;
    this.maxRecordingDuration = this.camera.cameraConfig.hsvRecordingDuration ?? 0;
    this.timeshiftedSegments = 0;
    this.transmittedSegments = 0;
    this.rtspEntry = null;
    this.timeshift = new TimeshiftBuffer(camera);
  ...
  public async *handleRecordingStreamRequest(streamId: number): AsyncGenerator<RecordingPacket> {
    ...
    if (this.camera.hksvRecording && this.isRecording && !this.isInitialized) {
    ...
    if (!this.camera.hksvRecording || !this.isInitialized || !(await this.startTransmitting()) || !this.ffmpegStream) {
      ...
      // Something's gone wrong, or we've disabled HKSV recording. In either event, we send an fMP4 stream header back to HKSV and exit as cleanly as we can.
      // If we can't get the stream header, we still send an empty segment to HKSV - this will still generate a warning in Homebridge that can be ignored.
    ...
    for await (const segment of this.ffmpegStream.segmentGenerator()) {
      ...
      // If we've exceeded a user-configured maximum recording duration, let HomeKit know we're stopping. We imperfectly calculate our recording duration by using
      // the fact that each transmitted segment will contain a single I-frame. The method is imperfect because partial segments happen, as well as other edge cases,
      // but it's more than good enough for our purposes.
  ...
  private async configureTimeshifting(): Promise<boolean> {
    ...
    this.rtspEntry = this.camera.findRecordingRtsp(this.recordingConfig.videoCodec.resolution[0], this.recordingConfig.videoCodec.resolution[1]);
    ...
    if (!this.camera.timeshift || !this.isRecording) {
    ...
    if (!(await this.camera.setBitrate(this.rtspEntry.channel.id, this.recordingConfig.videoCodec.parameters.bitRate * 1000))) {
  ...
  private async startTransmitting(): Promise<boolean> {
    ...
    //
    // We want to keep feeding HomeKit until it tells us it's finished, or we decide we don't want to send anymore fMP4 packets. We treat this in a similar way to
    // how a DVR works where you can pause live television, but it continues to buffer what's being broadcast until you're ready to watch it. This is the same idea.
    ...
    // const isAudioActive = (this.camera.cameraConfig.videoConfig?.audio &&
    ...
    this.ffmpegStream = new FfmpegRecordingProcess(this.camera, this.recordingConfig, this.rtspEntry, isAudioActive);
    this.isTransmitting = true;

    // Let the timeshift buffer know it's time to transmit and continue timeshifting.
    if (this.camera.timeshift) {
  ...
  private async stopTransmitting(reason?: HDSProtocolSpecificErrorReason): Promise<void> {
    ...
    if (this.camera.hksvRecording && this.transmittedSegments && this.rtspEntry) {
      ...
      this.log.debug('HomeKit Secure Video recorded %s %s %s motion event.', this.timeshiftedSegments ? 'a' : 'an approximately', recordedTime, timeUnit);

      // Once we've got a successful event, let's reset our error count.
      this.camera.HksvErrors = 0;
    }
    ...
    if ((reason !== undefined) && (reason !== HDSProtocolSpecificErrorReason.NORMAL)) {
      ...
      this.log.error('HomeKit Secure Video recording event ended early: %s', reasonDescription);
      ...
      if (this.camera.hksvRecording && (++this.camera.HksvErrors >= PROTECT_HKSV_MAX_EVENT_ERRORS)) {

        this.log.error('Reconnecting to the Protect controller after %s consecutive HomeKit Secure Video event recording errors. ' +
          ...
          'If these issues persist, you might want to consider restarting the Protect controller.', this.camera.HksvErrors);
