...
export class FfmpegOptions {
  ...
  private configureHwAccel(): boolean {
    ...
    let logMessage = '';
    const accelCategories = this.camera.hardwareTranscoding ? 'decoding and transcoding' : 'decoding';
    ...
    if (this.camera.hardwareDecoding) {
    ...
    if (this.camera.hardwareTranscoding) {
      ...
      switch (this.platform.hostSystem) {
        ...
        default:
          ...
          if (this.platform.codecSupport.hasHwAccel('qsv') &&
            ...
            this.camera.hardwareDecoding = true;
