...
import { EufyLivestream } from '../controller/LiveStream.js';

// UniFi Protect livestream timeshift buffer.
export class TimeshiftBuffer extends EventEmitter {

  private readonly accessory: PlatformAccessory;
  private buffer: Buffer[];
  private bufferSize: number;
  private channelId: number;
  private readonly livestream: EufyLivestream;
  ...
  constructor(camera: CameraAccessory) {
    ...
    this.livestream = new EufyLivestream(camera);
  ...
  public async start(channelId: number, lens = 0): Promise<boolean> {
    ...
    if (this.camera.stream?.hksv?.recordingConfiguration?.mediaContainerConfiguration.fragmentLength) {

      if ((this.segmentLength < 100) || (this.segmentLength > 1500) ||
        (this.segmentLength > (this.camera.stream.hksv?.recordingConfiguration?.mediaContainerConfiguration.fragmentLength / 2))) {
    ...
    if (!(await this.livestream.start(this.camera.accessory.UUID, channelId, lens, this.segmentLength))) {
